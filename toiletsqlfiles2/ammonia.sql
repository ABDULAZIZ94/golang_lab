...
WITH
    ...
    GENTIME as (
        ...
        FROM generate_series(
                date_trunc(
                    'HOUR', TO_TIMESTAMP(
                        '2024-11-13 16:00:00', 'YYYY-MM-DD HH24:MI:SS'
                    )
                ), date_trunc(
                    'HOUR', TO_TIMESTAMP(
                        '2024-11-14 15:59:59', 'YYYY-MM-DD HH24:MI:SS'
                    )
                ), interval '1 HOUR'
            ) uplinkTS
        where (
                (
                    uplinkTS::time >= '16:00:00'
                    OR (
                        uplinkTS::time >= '00:00:00'
                        AND uplinkTS::time <= '15:59:59'
                    )
                )
            )
    )
...
-- ammonia male
select * from ammonia_data where device_token = '73' order by timestamp desc