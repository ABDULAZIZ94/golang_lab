...
func main() {
	...
	for {
		...
		// time.Sleep(1 * time.Second)
		//publish smoke data
		mqtt.GetMqttClient().Publish("az/public/t2/110/uplink", 0, false, generateSmokeData())
		time.Sleep(1 * time.Second)
		...
		//publish panic data
		mqtt.GetMqttClient().Publish("az/public/t2/111/uplink", 0, false, generatePanicBtnData())
		time.Sleep(1 * time.Second)
